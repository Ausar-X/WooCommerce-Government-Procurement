...
int main()
{
	...
	cout << "Copyright 2020 Henan Chengyu Trading Co., Ltd. All rights reserved." << endl;
	cout << "\nOfficial Website: https://chengyutrading.com/" << endl;
	...
	cout << "Do you want to create or delete a product (press q to quit)? [c/d/q] ";
	...
	while (toupper(c_or_d[0]) != CD[2])
	{
		...
		if (toupper(c_or_d[0]) == CD[0])
		{
			...
			{
				postData = "pwd=YOUR_OWN_PASSWORD&productID=" + enxiProductID + "&productImg=" + imgURL + "&productPrice=" + productPrice;
			}
			if (productDesc != "")
			{
			...
			{
				PI_Tools::toClipboard("http://chengyutrading.com/shop/" + enxiProductID + "/");
		...
		{
			postData = "delete=yes&pwd=YOUR_OWN_PASSWORD&productURL=" + productURL;
			easy_curl.curl_send("https://api.example.com/", preHeaders, 0, WooCommerceRet, "POST", false, postData);
			if (WooCommerceRet == "OK")
			{
